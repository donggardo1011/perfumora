...
$sql = "SELECT * FROM products WHERE product_type = 'men' and quantity > 0";
$stmt = $pdo->query($sql);
?>
...
.nav-icon i:hover {
  ...
  color: #ffffff;
    ...
    .cardborder {
      ...
      border-radius: 10px;
      box-shadow: 0 10px 60px rgba(0, 0, 255, 0.7);
      text-align: center;
      padding: 15px;
      backdrop-filter: blur(10px);
      -webkit-backdrop-filter: blur(10px);
...
.view-btn {
  ...
  cursor: pointer;           /* Optional: pointer on hover */
    ...
    .custom-modal-bg {
  background: linear-gradient(to bottom right, #004aad, #2e82ff);
  border-radius: 12px;
      ...
      <div class="modal-body">
        <div class="row align-items-center">
          ...
          <div class="col-md-7">
            ...
            <h6><strong>Quantity:</strong> <?= ucfirst($row['quantity']) ?></h6>
            <p class="mt-3"><strong>Description:</strong><br><?= $row['product_description'] ?></p>
            <a href="../transactions/order.php?id=<?= $row['id'] ?>" class="btn btn-light text-primary font-weight-bold mr-2">Buy Now</a>
